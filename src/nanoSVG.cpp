...
#include <stdio.h>   // g++ wants this as well (for sscanf and FILE related use)
...
// this is just a copy of the nanosvg.h function code for testing purposes
// since I couldn't get my printf statements to do anything useful when
// put into the function code in the header
...
//	printf("%s: entry\n", __func__);
	...
	if (d < 1e-6f && rx > 0.0f && rx == ry) {
		// The arc degenerates to a circle
		float r = rx, cx = x2, cy = y2;
		// following calls copied from circle parsing func
		nsvg__resetPath(p);
		...
		*cpx = x2;
		*cpy = y2;
//		printf("%s: d=%f, rx=%f, ry=%f, rotx=%f, fa=%d, fs=%d, x1=%f, y1=%f, x2=%f, y2=%f\n",
//				__func__, d, rx, ry, rotx, fa, fs, x1, y1, x2, y2);
//		printf("%s: exit(1)\n", __func__);
		return;
	}
...
//							printf("%s: pathLineTo\n", __func__);
						...
						case 'H':
						case 'h':
//							printf("%s: pathHLineTo\n", __func__);
							nsvg__pathHLineTo(p, &cpx, &cpy, args, cmd == 'h' ? 1 : 0);
                            cpx2 = cpx; cpy2 = cpy;
							break;
						case 'V':
						case 'v':
//							printf("%s: pathVLineTo\n", __func__);
							nsvg__pathVLineTo(p, &cpx, &cpy, args, cmd == 'v' ? 1 : 0);
                            cpx2 = cpx; cpy2 = cpy;
							break;
						case 'C':
						case 'c':
//							printf("%s: pathCubicBezTo\n", __func__);
							nsvg__pathCubicBezTo(p, &cpx, &cpy, &cpx2, &cpy2, args, cmd == 'c' ? 1 : 0);
							break;
						case 'S':
						case 's':
//							printf("%s: pathCubicBezShortTo\n", __func__);
							nsvg__pathCubicBezShortTo(p, &cpx, &cpy, &cpx2, &cpy2, args, cmd == 's' ? 1 : 0);
							break;
						case 'Q':
						case 'q':
//							printf("%s: pathQuadBezTo\n", __func__);
							nsvg__pathQuadBezTo(p, &cpx, &cpy, &cpx2, &cpy2, args, cmd == 'q' ? 1 : 0);
							break;
						case 'T':
						case 't':
//							printf("%s: pathQuadBezShortTo\n", __func__);
...
//		printf("%s: linearGradient\n", __func__);
		nsvg__parseGradient(p, attr, NSVG_PAINT_LINEAR_GRADIENT);
	} else if (strcmp(el, "radialGradient") == 0) {
//		printf("%s: radialGradient\n", __func__);
		nsvg__parseGradient(p, attr, NSVG_PAINT_RADIAL_GRADIENT);
	} else if (strcmp(el, "stop") == 0) {
//		printf("%s: stop\n", __func__);
		nsvg__parseGradientStop(p, attr);
