...
/* gente addition */
/* #define GNOMECC_DATA_DIR GENTE_DATA_DIR */
#define GNOMECC_DATA_DIR "/usr/share/libtimezonemap"
...
/*		g_warning ("Could not open *%s*\n", tz_data_file);*/
	...
	while (fgets (buf, sizeof(buf), tzfile))
	{
		...
		*p = '\0';
		...
		/* DGL NOTE TODO: does the created GQuark need to be 'free'd? at some point */

		/* gente addition */
		/* this creates a GQuark so we can use g_quark_try_string()
		 * later and if there isn't already a quark for the timezone
		 * string, we know we won't find a TzLocation. */
		g_quark_from_string (loc->zone);
...
/*struct {*/ /* give it a name to get rid of the warning */
struct aliases_s {
	const char *orig;
	const char *dest;
} aliases[] = {
	...
	{ "CST",            "America/Chicago" },     /* Other name for the Central tz */
	{ "CST6CDT",        "America/Chicago" },     /* ditto */
	{ "MST",            "America/Denver" },      /* Other name for the Mountain tz */
	{ "MST7MDT",        "America/Denver" },      /* ditto */
	{ "PST",            "America/Los_Angeles" }, /* Other name for the Pacific tz */
	{ "PST8PDT",        "America/Los_Angeles" }, /* ditto */
};
...
/*	ret = g_hash_table_lookup (tz_db->backward, timezone); */
	ret = (char*)g_hash_table_lookup (tz_db->backward, timezone);
	if (ret == NULL)
